    ...
    for row in rp3:
        ...
        class Widget(QDialog):
            def __init__(self, parent=None):
                ...
                q11Edit.setFont(QFont("Arial",11))
                ...
                grid.addWidget(lbl12, 14, 1)
                grid.addWidget(q12Edit, 14, 2)
        ...
        def main(): 
            ...
            if row[3][0] == '1':
                updwerk = update(werken).where(werken.c.werknummerID == mwerknr).\
                  values(begr_leiding = maannsom)
                conn.execute(updwerk)
            elif row[3][0] == '2':
                 updwerk = update(werken).where(werken.c.werknummerID == mwerknr).\
                  values(begr_huisv = maannsom)
                 conn.execute(updwerk) 
            elif row[3][0] == '3':
                 updwerk = update(werken).where(werken.c.werknummerID == mwerknr).\
                  values(begr_materieel = maannsom)
                 conn.execute(updwerk)  
            elif row[3][0] == '4':
                 updwerk = update(werken).where(werken.c.werknummerID == mwerknr).\
                  values(begr_vervoer = maannsom)
                 conn.execute(updwerk)
            elif row[3][0] == '5':
                 updwerk = update(werken).where(werken.c.werknummerID == mwerknr).\
                  values(begr_inhuur = maannsom)  
                 conn.execute(updwerk) 
            elif row[3][0] == '6':
                 updwerk = update(werken).where(werken.c.werknummerID == mwerknr).\
                  values(begr_overig = maannsom)
                 conn.execute(updwerk) 
            elif row[3][0] == '7':
                 updwerk = update(werken).where(werken.c.werknummerID == mwerknr).\
                  values(begr_beton = maannsom)
                 conn.execute(updwerk)
            elif row[3][0] == '8':
                 updwerk = update(werken).where(werken.c.werknummerID == mwerknr).\
                  values(begr_kabelwerk = maannsom)
                 conn.execute(updwerk)
            elif row[3][0] == '9':
                 updwerk = update(werken).where(werken.c.werknummerID == mwerknr).\
                  values(begr_grondverzet = maannsom)
                 conn.execute(updwerk)
