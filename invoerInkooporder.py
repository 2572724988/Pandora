...
def foutArtikel(mregel):
    msg = QMessageBox()
    msg.setStyleSheet("color: black;  background-color: gainsboro")
    msg.setIcon(QMessageBox.Critical)
    msg.setText('Artikelnummer onjuist\norderregel '+str(mregel)+'\nis niet ingevoerd!')
    msg.setWindowTitle('Bestelling regel inkooporder')
    msg.exec_()
    
def onvBesteld(aantal, mresrgl):
    msg = QMessageBox()
    msg.setStyleSheet("color: black;  background-color: gainsboro")
    msg.setIcon(QMessageBox.Critical)
    msg.setText(str(aantal)+' te weinig besteld voor bestelregel '+ str(mresrgl))
    msg.setWindowTitle('Bestelling regel inkooporder')
    msg.exec_()
...
def inkoopRegels(minkgeg, mregel):
    ...
    class Widget(QDialog):
        def __init__(self, parent=None):
            ...
            grid.addWidget(QLabel('\u00A9 2017 all rights reserved dj.jansen@casema.nl'), 12, 0, 1, 3, Qt.AlignCenter)
    ...
    conn.execute(insrgl)
    selart = select([artikelen]).where(artikelen.c.artikelID == martikelnr)
    if conn.execute(selart):
        updart = update(artikelen).where(and_(orders_inkoop_artikelen.c.artikelID == \
              ...
              .values(bestelsaldo = artikelen.c.bestelsaldo + mhoev, bestelstatus = True)
        conn.execute(updart)
    else:
        foutInvoer()
        inkoopRegels(minkgeg, mregel)
    ...
    for row in rpmat:
        ...
        if mhoev <= 0:
            ...
            onvBesteld(row[3] - mhoev, mresrgl)
        mresrgl += 1
