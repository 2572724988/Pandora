...
def toonGrafiek(m_email):
    ...
    mjrmndvj = str(int(str(date.today())[0:4])-1)+'-'+str(int(str(date.today())[5:7])+1)
    if len(mjrmndvj) == 6:
        mjrmndvj = str(int(str(date.today())[0:4])-1)+'-0'+str(int(str(date.today())[5:7])+1)
    metadata = MetaData()
...
def magVoorraad(m_email):
    ...
    header = ['Jaar-Maand', 'Totaal Voorraad', 'Courante Vorraad', 'Incourante Voorraad']    
